...
def try_to_expand_v(size, cur_gp, mrk, vs):
    bs = vs[utils.set_hash(cur_gp)]
    for i in range(size):
        if mrk[i]:
            continue
        cur_gp[i] = True
        if vs[utils.set_hash(cur_gp)] >= bs:
            return i
        cur_gp[i] = False
    return None


def find_fc_v(size, vs):
    F = []
    tot_num = 0
    mrk = np.zeros(size, dtype=bool)
    while tot_num < size:
        ...
        while True:
            F.append(cur_gp.copy())
            print(cur_num)
            nxt = try_to_expand_v(size, cur_gp, mrk, vs)
            if nxt is None:
                break
            cur_gp[nxt] = mrk[nxt] = True
            cur_num += 1
